  ...
  >
    <div class="modal-box w-full max-w-5xl max-h-[90vh] flex flex-col p-0 shadow-2xl">
      ...
      <div class="relative overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-br from-primary to-primary-focus opacity-90"></div>
        <div class="relative flex items-center justify-between p-8">
          <div>
            <h2 class="text-3xl font-bold text-primary-content mb-1">Device Manager</h2>
            <p class="text-primary-content/70 text-sm">Manage groups, devices, and resources</p>
          </div>
          <button
            @click="$emit('close')"
      ...
      <div class="flex-1 overflow-hidden flex flex-col bg-base-100">
        <!-- Tab Navigation -->
        <div role="tablist" class="tabs tabs-boxed mx-6 mt-6 mb-4 bg-base-200 p-1 gap-1">
          <button
            role="tab"
        ...
        <!-- Device Groups Tab -->
        <div v-if="activeTab === 'groups'" class="flex-1 overflow-y-auto px-6 pb-6">
            ...
            >
              <div class="card-body p-4 flex flex-row items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="card-title text-base mb-1">{{ group.name }}</h4>
        ...
        <!-- Devices Tab -->
        <div v-if="activeTab === 'devices'" class="flex-1 overflow-y-auto px-6 pb-6">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h3 class="text-lg font-semibold">Custom Devices</h3>
          ...
          <div class="grid gap-3">
            ...
            >
              <div class="card-body p-4 flex flex-row items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="card-title text-base mb-2">{{ device.name }}</h4>
                  ...
                  <p v-if="device.description" class="text-xs opacity-70 line-clamp-2">
                    {{ device.description }}
                  </p>
                </div>
        ...
        <div v-if="activeTab === 'providers'" class="flex-1 overflow-y-auto px-6 pb-6">
          <div class="alert alert-info mb-6 shadow-md">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <div>
              <h3 class="font-bold">About Resource Providers</h3>
              <div class="text-sm">Infrastructure components that add capacity to your site (PDUs, HVAC units, network uplinks, etc.)</div>
            </div>
          </div>

          <div class="flex items-center justify-between mb-6">
            <div>
              <h3 class="text-lg font-semibold">Resource Providers</h3>
          ...
          <div class="grid gap-3">
            ...
            >
              <div class="card-body p-4 flex flex-row items-center gap-4">
                <div class="flex flex-col items-center justify-center w-20 h-20 rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 flex-shrink-0">
                  <svg v-if="provider.type === 'power'" class="w-10 h-10 text-primary" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd" />
                  </svg>
                  <svg v-else-if="provider.type === 'cooling'" class="w-10 h-10 text-primary" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex flex-wrap gap-2 mb-2">
                    <span v-if="provider.powerCapacity > 0" class="badge badge-primary badge-sm">{{ provider.powerCapacity }}W</span>
                    <span v-if="provider.powerPortsCapacity > 0" class="badge badge-primary badge-sm">{{ provider.powerPortsCapacity }} ports</span>
                    <span v-if="provider.coolingCapacity > 0" class="badge badge-accent badge-sm">{{ (provider.coolingCapacity / 12000).toFixed(1) }} Tons</span>
                    <span v-if="provider.networkCapacity > 0" class="badge badge-secondary badge-sm">{{ provider.networkCapacity }} Gbps</span>
                  </div>
                  <div v-if="provider.location" class="text-xs opacity-70 mb-1">
                    📍 {{ provider.location }}
                  </div>
                  <p v-if="provider.description" class="text-xs opacity-70 line-clamp-1">
            ...
            <div v-if="resourceProviders.length === 0" class="text-center py-12 opacity-50">
              <svg class="w-16 h-16 mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
              </svg>
              <p class="text-lg font-semibold mb-2">No resource providers yet</p>
              <p class="text-sm">Add providers to supply power, cooling, and network capacity</p>
            </div>
          </div>
        </div>

        <!-- Import/Export Tab -->
        <div v-if="activeTab === 'import-export'" class="flex-1 overflow-y-auto px-6 pb-6">
              ...
              <div class="card-body">
                ...
                <div class="stats shadow mb-4 bg-base-100">
                  <div class="stat py-3 px-4">
                    <div class="stat-title text-xs">Device Groups</div>
                    <div class="stat-value text-2xl text-primary">{{ deviceGroups.length }}</div>
                  </div>
                  <div class="stat py-3 px-4">
                    <div class="stat-title text-xs">Custom Devices</div>
                    <div class="stat-value text-2xl text-secondary">{{ customDevices.length }}</div>
                  </div>
                </div>
              ...
              <div class="card-body">
                ...
                <div class="form-control mb-4">
                  ...
                  <div class="flex gap-2">
                    <label class="btn btn-sm flex-1 cursor-pointer" :class="importMode === 'merge' ? 'btn-primary' : 'btn-outline'">
                      <input
                        type="radio"
                        v-model="importMode"
                        value="merge"
                    ...
                    <label class="btn btn-sm flex-1 cursor-pointer" :class="importMode === 'replace' ? 'btn-warning' : 'btn-outline'">
                      <input
                        type="radio"
                        v-model="importMode"
                        value="replace"
                ...
                <div v-if="importError" class="alert alert-error mt-4 shadow-md">
    ...
    <div class="modal-box w-full max-w-md max-h-[90vh]">
      ...
      <div style="max-height: 60vh; overflow-y: auto;">
        ...
        <div class="mb-4">
          ...
          <label class="label">
            <span v-if="validationErrors.category" class="label-text-alt text-error">{{ validationErrors.category }}</span>
            <span v-else class="label-text-alt">Create new groups in the "Device Groups" tab</span>
          </label>
        </div>
        ...
        <div class="mb-4">
          ...
          />
          <label class="label">
            <span v-if="validationErrors.ruSize" class="label-text-alt text-error">{{ validationErrors.ruSize }}</span>
            <span v-else class="label-text-alt">1U = 1.75 inches (44.45mm)</span>
          </label>
        </div>
        ...
        <div class="mb-4">
          ...
          />
          <label class="label">
            <span v-if="validationErrors.powerDraw" class="label-text-alt text-error">{{ validationErrors.powerDraw }}</span>
            <span v-else class="label-text-alt">Maximum power consumption under load</span>
          </label>
        </div>
        ...
        <div class="mb-4">
          ...
          />
          <label class="label">
            <span v-if="validationErrors.powerPortsUsed" class="label-text-alt text-error">{{ validationErrors.powerPortsUsed }}</span>
            <span v-else class="label-text-alt">Number of PDU power ports required (1 for single PSU, 2 for dual PSU, etc.)</span>
          </label>
        </div>
